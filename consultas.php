...
class Moeda {
     ...
     * @param string $url
     */
    function __construct($username , $password , $host , $port, $url = null) {
    ...
    function __call($method, $params) {
        ...
        if ($this->response['error']) {
            // If axed returned an error, put that in $this->error
            $this->error = $this->response['error']['message'];
        }
        elseif ($this->status != 200) {
            // If axed didn't return a nice error message, we need to make our own
            switch ($this->status) {
                case 400:
...
$moeda = new Moeda($username,$password,$host,$port);

$getinfo = $moeda->getinfo();
$conexoes = $getinfo['connections'];
$blocos = $getinfo['blocks'];
$walletVersion = $getinfo['version'];
$protocolVersion = $getinfo['protocolversion'];
...
$latest_price_spero_brl = $result_spero_current['brl'];
$latest_price_spero_usd = $result_spero_current['usd'];
